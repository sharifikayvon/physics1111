...
if mode == "Upload data file":
    ...
    y_str = st.text_area("Y values", "2 4 1 3 5")
...
if 'xdata' not in locals() and 'ydata' not in locals():
    xdata = np.arange(0, 4*np.pi, 0.05)
    ydata = 5 * np.sin(2*xdata) + 1.0 + np.random.normal(0, .5, len(xdata))
...
if fitline:
    ...
    ax.plot(lin_xfit, lin_yfit, color="lime", linestyle="--", label=lin_label)
    ax.legend()
